...
const StaffList = () => {
  ...
  // Replace your current handleSubmitReview with this improved version
  const handleSubmitReview = async () => {
    if (!selectedStaff) return;

    // Enhanced Validation
    if (reviewRating < 1) {
      toast.error("Please select a star rating (at least 1 star)");
      return;
    }
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/client/add-review`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok && result.status === true) {
        ...
        if (result.errors?.star) {
          toast.error(result.errors.star[0]);
        } else {
          toast.error(result.message || "Failed to submit review");
        }
        console.error("Review submission errors:", result.errors);
      }
    } catch (error) {
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      ...
                      className={`transition-all ${isViewMode ? "cursor-default" : "cursor-pointer hover:scale-110"}`}
                    >
                      <Star
                        className={`w-9 h-9 transition-colors ${
                          star <= reviewRating
                            ? "fill-[#FAB435] text-[#FAB435]"
                            : "text-[#E0E0E0] hover:text-[#FAB435]/50"
                        }`}
                      />
                    </button>
                  ))}
                </div>
                {reviewRating === 0 && !isViewMode && (
